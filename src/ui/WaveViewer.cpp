    ...
    _drawPressure( _solver.N() )
{
  ...
  VEC3F sceneCenter = solver.sceneCenter();
...
void WaveViewer::drawMesh()
{
  ...
  for ( int mesh_idx = 0; mesh_idx < meshes.size(); mesh_idx++ )
  {
    ...
    for ( int i = 0; i < triangles.size(); i++ )
    {
      ...
      const VEC3F             &n = triangles[ i ]->getNormal();

      for ( int j = 0; j < 3; j++ )
      {
        const VEC3F           &v = triangles[ i ]->getX( j );
...
void WaveViewer::drawReceivers()
{
  ...
  {
    const VEC3F             &pos = receivers->at( listen_idx );
...
//////////////////////////////////////////////////////////////////////
VEC3F WaveViewer::computeVertexColour( const Tuple3i &index )
{
  Real                       pressure;

  VEC3F                      colour( 0.2, 0.2, 0.2 );
