         ...
         class KDTree : private boost::noncopyable
{
    ...
    void sift_down(std::vector<int>& pid,
            const size_t n,
            std::vector<typename _Acc::result_type> &auxHeap) const
    {
        typename _Acc::result_type a = auxHeap[0];
        int ia = pid[0];
        int j = 1, jold = 0;
        while (j < (int)n)
        {
            if ( j < (int)n-1 && auxHeap[j] < auxHeap[j+1] ) ++ j;
            if ( a >= auxHeap[j] ) break;

            auxHeap[jold] = auxHeap[j];
            pid[jold] = pid[j];
     ...
     */
        KDTree(const _Val* data, const size_t N, bool copy = false):
            m_N(0), m_numNodes(0), m_copied(false)
    {
        ...
        void find_nearest(const _Val& pt, const size_t n, std::vector<int>& ret) const
        {
            ...
            std::vector<typename _Acc::result_type> auxHeap(n);
            ret.resize(n);
            std::fill(auxHeap.begin(), auxHeap.end(), typename _Acc::result_type(LARGE_VAL));
            ...
            while ( ntask )
            {
                ...
                int k = task[ntask --];
                if ( k == nodeId ) continue;
                if ( dist_sqr_to_node(k, pt) < auxHeap[0] )
                {
                    if ( m_nodes[k].child[0] )
                    {
                        assert(ntask < 128 && ntask >= 0);
         ...
         */
                    ...
                    {
                        out.clear();
                        typename _Acc::result_type ZERO(0);
                        if ( r < ZERO ) return 0;
                     ...
                     */
                        void reinitialize(const _Val* data, const size_t N, bool copy = false)
                        {
                            ...
                            //LOGGING_DEBUG("free %d tree node\n", (int)m_numNodes);
                            ...
                            //LOGGING_DEBUG("allocate %d tree node\n", (int)m_numNodes);
                            ...
                            for(int i = 0;i < _Dim;++ i)
                            {
                                m_nodes[0].bdBox[0][i] = typename _Acc::result_type(-LARGE_VAL);
                                m_nodes[0].bdBox[1][i] = typename _Acc::result_type(LARGE_VAL);
                            }
