...
#endif
{
    ...
    {
        const int kStart = floor(float(sliceIdx) / SLICES * (resolution+1));
        const int kEnd = floor(float(sliceIdx + 1) / SLICES * (resolution+1));
        
        double radius = len(side);

        // do the zig-zag
        int i=0, j=0, k=kStart;
    ...
    //#endif
        ...
        while (k < kEnd);  //resolution);
        
        #pragma omp critical
        {
            int percent = floor(float(slicesDone) / SLICES * 100.0);
            slicesDone++;
            if (floor(float(slicesDone) / SLICES * 100.0) > percent)
                cout << percent << "% " << flush;
        }
...
#endif
{
    ...
    //#endif
        ...
        #pragma omp critical
        {
            int percent = floor(float(slicesDone) / SLICES);
            slicesDone++;
            if (floor(float(slicesDone) / SLICES > percent))
                cout << percent << "% " << flush;
        }
