        ...
        const Matrix3<REAL>& F, Matrix3<REAL>& out) const
{
    ...
    out = Matrix3<REAL>::identity(m_lambda * E.trace()) + 
        (REAL)2 * m_mu * E;
}
...
REAL StVKMaterial::strain_energy_density(const Matrix3<REAL>& F) const
{
    ...
    return 0.5*m_lambda*M_SQR(tr) + m_mu*E.colon(E); 
}
        ...
        Matrix<REAL>& out) const
{
    ...
    const REAL t66 = m_lambda*(t9/2.0+t18/2.0+t27/2.0-3.0/2.0+t33/2.0+t39/2.0+t45/2.0+t51/2.0+t57/2.0+t63/2.0);
    const REAL t70 = t66+m_mu*(t9+t18+t27-1.0);
    const REAL t71 = t1*t70;
    const REAL t73 = -iDm(0,1)-iDm(1,1)-iDm(2,1);
    const REAL t75 = -iDm(0,2)-iDm(1,2)-iDm(2,2);
    const REAL t78 = m_lambda*(t8*t1+t32*t73+t50*t75);
    const REAL t79 = m_mu*t8;
    const REAL t84 = t73*m_mu;
    const REAL t88 = t8*t32+t17*t38+t26*t44;
    const REAL t90 = t84*t88;
    const REAL t91 = t32*m_mu;
    const REAL t94 = t1*t32+t8*t73;
    const REAL t97 = t75*m_mu;
    const REAL t101 = t8*t50+t17*t56+t26*t62;
    const REAL t103 = t97*t101;
    const REAL t104 = t50*m_mu;
    const REAL t107 = t1*t50+t8*t75;
    const REAL t112 = t1*m_mu;
    const REAL t114 = t112*t88;
    const REAL t120 = t66+m_mu*(t33+t39+t45-1.0);
    ...
    const REAL t322 = m_lambda*(t17*iDm(0,0)+t38*iDm(0,1)+t56*iDm(0,2));
    ...
    const REAL t367 = m_lambda*(t26*iDm(0,0)+t44*iDm(0,1)+t62*iDm(0,2));
    ...
    const REAL t475 = m_lambda*(t17*iDm(1,0)+t38*iDm(1,1)+t56*iDm(1,2));
    ...
    const REAL t520 = m_lambda*(t26*iDm(1,0)+t44*iDm(1,1)+t62*iDm(1,2));
    ...
    const REAL t628 = m_lambda*(t17*iDm(2,0)+t38*iDm(2,1)+t56*iDm(2,2));
    ...
    const REAL t673 = m_lambda*(t26*iDm(2,0)+t44*iDm(2,1)+t62*iDm(2,2));
