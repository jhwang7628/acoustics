    ...
    _PML_absorptionStrength( 0.0 )
{
  ...
  xMin = bbox.minBound() - VEC3F( cellSize / 2.0, 0.0, 0.0 );
  yMin = bbox.minBound() - VEC3F( 0.0, cellSize / 2.0, 0.0 );
  zMin = bbox.minBound() - VEC3F( 0.0, 0.0, cellSize / 2.0 );
...
#pragma omp parallel for schedule(static) default(shared)
        ...
        interfacial_cell_idx++ )
  {
    ...
    VEC3F                x = field.cellPosition( cell_idx );
...
#pragma omp parallel for schedule(static) default(shared)
        ...
        bulk_cell_idx++ )
  {
    ...
    int                  neighbour_idx;
    Tuple3i              cell_indices = field.cellIndex( cell_idx );
    VEC3F                cell_position = field.cellPosition( cell_indices );
  ...
  for ( int dimension = 0; dimension < 3; dimension++ )
  {
          ...
          cell_idx++ )
    {
                ...
                && !_isBulkCell[ pressure_cell_idx2 ] )
      {
        ...
        VEC3F normal( 0.0, 0.0, 0.0 );
        VEC3F position = _velocityField[ dimension ].cellPosition( cell_idx );
        VEC3F gradient = _boundaryFields[ boundaryObject ]->gradient( position );

        normal[ dimension ] = -1.0;
        gradient.normalize();

        Real coefficient = normal * gradient;
...
//////////////////////////////////////////////////////////////////////
Real MAC_Grid::PML_absorptionCoefficient( const VEC3F &x, Real absorptionWidth,
                                          int dimension )
{
  //return 0.0;
