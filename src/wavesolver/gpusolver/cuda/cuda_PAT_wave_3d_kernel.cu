...
/* Get centroid position on the device */
...
__global__ void cuda_pat_wave_3d_velocity_kernel(Number_t * __restrict__ u,
	...
	__shared__ Number_t cache[RRADX+2][RRADY+2]; // RRADX = RRADY = 16
...
__global__ void cuda_pat_wave_3d_pressure_kernel(Number_t * __restrict__ u,
    ...
    const Number_t bx = w_get_pos(i, nx, xmin, xmax, dx); // nodal position
    //const Number_t abs_x = pat_wave_3d_absortion(bx+dx/2, xmin, xmax, pml_strength, pml_width);
    const Number_t abs_x = pat_wave_3d_absortion(bx, xmin, xmax, pml_strength, pml_width);
    ...
    const Number_t abs_y = pat_wave_3d_absortion(by, ymin, ymax, pml_strength, pml_width);
